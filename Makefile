...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name fanet-operator-builder
	$(CONTAINER_TOOL) buildx use fanet-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm fanet-operator-builder
	rm Dockerfile.cross
...
KIND_CLUSTER_NAME ?= fanet-cluster
KIND_VERSION ?= v0.20.0
KIND_NODE_IMAGE_VERSION ?= v1.27.3
KIND_IN_PATH := $(shell command -v kind 2>/dev/null)
KIND = $(if $(KIND_IN_PATH),$(KIND_IN_PATH),./bin/kind)

.PHONY: kind-config ensure-kind kind-cluster delete-cluster kind-load kind-deploy kind-load

.PHONY: ensure-kind
ensure-kind:
	@if ! [ -x "$(KIND)" ]; then \
		echo "INFO: kind not found in PATH or locally. Downloading..."; \
		mkdir -p ./bin; \
		curl -Lo ./bin/kind https://kind.x-k8s.io/dl/$(KIND_VERSION)/kind-$$(uname)-amd64; \
		chmod +x ./bin/kind; \
	fi
...
kind-config:
	@if ! [ -f "./kind-config.yaml" ]; then \
		echo "INFO: Creating default kind-config.yaml..."; \
		printf "kind: Cluster\napiVersion: kind.x-k8s.io/v1alpha4\nnodes:\n- role: control-plane\n- role: worker\n- role: worker" > ./kind-config.yaml; \
	fi

kind-cluster: ensure-kind kind-config
	@if ! $$( $(KIND) get clusters | grep -q "^$(KIND_CLUSTER_NAME)$$" ); then \
		echo "INFO: Creating kind cluster '$(KIND_CLUSTER_NAME)'..."; \
		$(KIND) create cluster --image kindest/node:$(KIND_NODE_IMAGE_VERSION) --config kind/config.yaml; \
	else \
		echo "INFO: Kind cluster '$(KIND_CLUSTER_NAME)' already exists."; \
	fi

delete-cluster:
	@if [ -x "$(KIND)" ] && $$(./bin/kind get clusters | grep -q "^$(KIND_CLUSTER_NAME)$$"); then \
...
kind-deploy: kind-cluster kind-load deploy
